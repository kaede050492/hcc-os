-- HCC OS v1.4 shared GUI runtime.
local Runtime={}
...
function Runtime.new(context)
 ...
 E.HCC_VERSION="1.4.0"; E.HCC_VERSION_LABEL="HCC OS v1.4.0"
 ...
 E.box=function(x,y,w,h) return {x=math.floor(x),y=math.floor(y),w=math.max(0,math.floor(w)),h=math.max(0,math.floor(h))} end
 E.intersect=function(a,b) local x,y=math.max(a.x,b.x),math.max(a.y,b.y); local r,t=math.min(a.x+a.w,b.x+b.w),math.min(a.y+a.h,b.y+b.h); if r<=x or t<=y then return nil end; return E.box(x,y,r-x,t-y) end
 E.union=function(a,b) local x,y=math.min(a.x,b.x),math.min(a.y,b.y); return E.box(x,y,math.max(a.x+a.w,b.x+b.w)-x,math.max(a.y+a.h,b.y+b.h)-y) end
 E.inside=function(r,x,y) return x>=r.x and y>=r.y and x<r.x+r.w and y<r.y+r.h end
